import { IDateTimeEventRowDataSchema, dateEventRowDataSchema, dateTimeEventRowDataSchema } from "@/types/row-data-types";
import { dateEventSchema, dateTimeEventSchema } from "../types/event-types";
		...
		data.forEach((event) => {
			if (event.start?.date !== null && event.start?.date !== undefined) {
				...
				eventContainer.dateEvents.push(
					dateEventRowDataSchema.parse({start:startDate, end:endDate,...rest})
				...
				eventContainer.dateTimeEvents.push(
					dateTimeEventRowDataSchema.parse({start:startDateTime, end:endDateTime,...rest})
				);
			} else {
				console.warn(
