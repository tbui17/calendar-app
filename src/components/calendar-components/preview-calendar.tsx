...
export const PreviewCalendarApp = () => {
	...
	const defaultData2: ITransformedEvent[] = [
		{
			id: "155",
			description: "Meeting",
	...
	const convertDate = (data: ICellRendererParams<Date, Date>, type:string) => {
    if (type === "date") return moment(data.value).format("MM-DD-YYYY");
    if (type === "datetime"){return moment(data.value).format("MM-DD-YYYY hh:mm A");}
	...
	const defaultColumnDefs: ColDef[] = [
		...
		{ field: "summary", sortable: true, filter: true, editable: true },
		{
			...
			cellRenderer: (params:ICellRendererParams) => convertDate(params, params.data.type),
			editable: true,
	...
	return (
		<div>
			<div>
				<div>
					<p>
						Must use Google API with signed in version to use date
						range functionality.
					</p>
				</div>
